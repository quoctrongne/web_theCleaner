...
// Kết nối đến database để lấy dữ liệu giá
...
// Truy vấn cho dịch vụ nhà ở
$query_home = "SELECT 
              s.name as service, 
              CASE 
                  WHEN sp.max_area IS NULL THEN CONCAT('Trên ', sp.min_area, 'm²') 
                  WHEN sp.min_area = 0 THEN CONCAT('Dưới ', sp.max_area, 'm²')
                  ELSE CONCAT(sp.min_area, 'm² - ', sp.max_area, 'm²') 
              END as area,
              CONCAT(FORMAT(sp.base_price, 0), 'đ/m²') as price
           FROM service_pricing sp
           JOIN services s ON sp.service_id = s.serviceID 
           WHERE s.service_code = 'home'
           ORDER BY sp.min_area";
           
// Truy vấn cho dữ liệu giá tính toán JavaScript
...
try {
    $homePricing = db_get_rows($query_home);
    ...
    if (empty($homePricing)) {
        throw new Exception("Không có dữ liệu cho dịch vụ vệ sinh nhà ở");
    }
} catch (Exception $e) {
    // Dữ liệu mặc định khi truy vấn thất bại
    $homePricing = [
...
// Truy vấn cho dịch vụ văn phòng
$query_office = "SELECT 
                s.name as service, 
                CASE 
                    WHEN sp.max_area IS NULL THEN CONCAT('Trên ', sp.min_area, 'm²') 
...
// Thử lấy dữ liệu từ database
try {
    $officePricing = db_get_rows($query_office);
    
    // Dữ liệu cho JavaScript
    $officePricingJS = db_get_rows($query_office_pricing);
    
    // Nếu không có dữ liệu từ database, sử dụng dữ liệu mặc định
    if (empty($officePricing)) {
        throw new Exception("Không có dữ liệu cho dịch vụ vệ sinh văn phòng");
    }
} catch (Exception $e) {
    // Dữ liệu mặc định khi truy vấn thất bại
    $officePricing = [
