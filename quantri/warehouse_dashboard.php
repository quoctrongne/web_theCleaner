...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cleanerInventory'])) {
    ...
    $sqlCleanerInventory = "INSERT INTO CleanerInventory (employeeID, itemName, quantity, inventoryDate) VALUES ($employeeID, '$itemName', $quantity, CURDATE())";
    if ($conn->query($sqlCleanerInventory) !== TRUE) {
        echo "Lỗi: " . $sqlCleanerInventory . "<br>" . $conn->error;
    }
}

// Lấy dữ liệu báo cáo kho hàng ngày
$dailyReport = [];
$sqlDailyReport = "SELECT Employees.fullName, CleanerInventory.itemName, SUM(CleanerInventory.quantity) AS totalQuantity 
                    FROM CleanerInventory 
                    JOIN Employees ON CleanerInventory.employeeID = Employees.employeeID 
                    WHERE CleanerInventory.inventoryDate = CURDATE() 
                    GROUP BY Employees.fullName, CleanerInventory.itemName";
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Kiểm kê vật tư cho nhân viên vệ sinh</h6>
                        </div>
                        <div class="card-body">
                            <form method="POST">
                                <div class="form-group">
                                    <label>Nhân viên vệ sinh:</label>
                                    <select name="employeeID" class="form-control" required>
                                        <?php
                                        // Lấy danh sách nhân viên vệ sinh từ cơ sở dữ liệu
                                        $sqlEmployees = "SELECT employeeID, fullName FROM Employees WHERE role = 'Cleaner'";
