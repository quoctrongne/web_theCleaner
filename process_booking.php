...
function calculateServicePriceFromDB($service_id, $area) {
    ...
    // Truy vấn chi tiết để lấy giá theo diện tích cụ thể
    $query = "SELECT * FROM service_pricing 
              WHERE service_id = :service_id 
              AND min_area <= :area 
              AND (max_area IS NULL OR max_area >= :area)";
              
    // Thêm điều kiện ưu tiên bảng giá có phạm vi cụ thể (có max_area)  
    $query .= " ORDER BY CASE 
                  WHEN max_area IS NOT NULL THEN 1 
                  ELSE 2 
                END, min_area DESC 
                LIMIT 1";
    ...
    if (!$pricing) {
        // Nếu không tìm thấy giá cụ thể, thử tìm giá áp dụng cho tất cả diện tích
        $fallback_query = "SELECT * FROM service_pricing 
                  WHERE service_id = :service_id 
                  AND min_area = 0 
                  AND max_area IS NULL 
                  LIMIT 1";
        $pricing = db_get_row($fallback_query, $params);
        
        if (!$pricing) {
            error_log("Không tìm thấy cấu hình giá, sử dụng giá mặc định theo bảng giá hiển thị");
            
            // Giá mặc định dựa vào bảng giá trong hình ảnh
    ...
    // Tính giá dựa trên pricing_type từ database
    if ($pricing['pricing_type'] === 'per_sqm') {
        // Giá theo m²
        $price = $area * floatval($pricing['base_price']);
    } else {
        // Giá cố định
        $price = floatval($pricing['base_price']);
